...
O [**_Folium_**](https://python-visualization.github.io/folium/index.html) é um pacote que possibilita a criação de mapas _online_, facilitando a visualização dos dados manipulados no _Python_ em um mapa que usa a biblioteca JavaScript [**_Leaflet_**](https://leafletjs.com). É possível inserir _basemaps_, _shapefiles_, arquivos _geojson_ e _topojson_.
...
Diversos outros podem auxiliar, tais como _branca_, _geopandas_, _seaborn_ etc.
...
PS: Um site relevante para obter as cores em formato hexadecimal (bastante utilizado na definição dos _styles_ do **folium** é usando o site [ColorBook.io](https://www.colorbook.io)).
...
O mapa pode ter diferentes _basemaps_, que são, na essência, o mapa de fundo renderizado em _titles_. O _folium_ utiliza, por _default_, o basemap do _OpenStreetMap_, contudo existe a possibilidade de adicionar outros serviços, conforme se vê abaixo.
...
É possível alterar o _.html_ gerado, acrescentando customizações diretamente no arquivo.
...
Fiz a tentativa de usar o domínio [**_raw.githubusercontent.com_**](https://raw.githubusercontent.com) sem sucesso, que possibilita acessar diretamente, porém recebi a seguinte mensagem de erro ao tentar renderizar o conteúdo do _.html_.
